...
| CIS-Docker-4.12 | Build/Release | Ensure all signed artifacts are validated. | `SATISFIED_REPO_CONTROL` | `tools/build_app.sh` verifies the upstream signed checksum file with the published signing key before validating application archives and binaries. The retained `publish-image.yaml` defines steps to sign and verify a pushed image digest, but those release steps are not operational while the workflow is manual-dispatch-only and the publish job requires a `push` event; release-path coverage is pending replacement by the redesigned pipeline. |
